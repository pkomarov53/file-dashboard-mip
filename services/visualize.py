...
def render_dashboard(df):
    ...
    # Вкладки
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Учебный процесс", 
                                      "Платформа и Наставник", 
                                      "Карьера и интересы", 
                                      "Открытые ответы",
                                      "Важные уточнения"
                                    ])
    ...
    with tab5:
        st.markdown("1. В фильтрах пола студента вариант «Не указано» соответствует **пропущенным значениям.** " \
        "Такой подход позволяет одновременно учитывать NaN-значения для минимального расхождения статистики")
